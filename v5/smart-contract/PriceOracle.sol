...
contract PriceOracle is IexecDoracle, Ownable, IOracleConsumer, IERC2362
{
    ...
    mapping(bytes32 => TimedValue) private values;
	...
	external override
	{
        ...
        TimedValue storage prior = values[id];

        // Check timestamp
        require(timestamp > prior.date, "Old timestamp");
        values[id] = TimedValue(_callID, timestamp, value);
	...
	function get(string calldata key) external view returns (uint64, uint64) {
        TimedValue storage datum = values[keccak256(bytes(key))];
    ...
    function getCallID(string calldata key) external view returns (uint64) {
        return values[keccak256(bytes(key))].oracleCallID;
    }
